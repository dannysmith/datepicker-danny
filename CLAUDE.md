...
- [x] Ensure the datepicker is robust and self-contained in various curcumstances. Consider container queries, and isolating certain parts of the CSS so they aren't fucked by inheritance from external global CSS. We should think about when and where we want to do this though, because we probably do want this, if it's used in a site with a certain font, to inherit that font. There may be trade offs to decide here. 
